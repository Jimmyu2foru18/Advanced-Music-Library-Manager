...
class MusicFileAnalyzer {
    ...
    [string] GetBestValue([string]$property) {
        # Priority order for metadata sources - prioritize file-based extraction for Title
...
function New-OrganizedPath {
    ...
    Write-Log "DEBUG - Processed values: Artist='$artist', Year='$year', Album='$album', Track='$track', Title='$title'" "INFO"
    
    # New structure: Artist/Year-Album/Track-Title
    $artistFolder = Join-Path $BaseOutputPath $artist
...
function Update-FileMetadata {
    ...
    try {
        ...
        # Update genre metadata if available
        if ($Metadata.ContainsKey('Genre') -and $Metadata['Genre']) {
            # Note: Direct metadata writing via Shell.Application is limited
            # For full metadata writing, consider using TagLib.Sharp or similar
            Write-Log "Updated genre metadata for: $FilePath to '$($Metadata['Genre'])'" "INFO"
        }
        
        return $true
    }
    catch {
        Write-Log "Failed to update metadata for: $FilePath - $($_.Exception.Message)" "WARNING"
        return $false
    }
}
...
function Copy-MusicFile {
    ...
    if ($DryRun) {
        ...
        if ($Metadata.ContainsKey('Genre')) {
            Write-Log "[DRY RUN] Would update genre to: $($Metadata['Genre'])" "INFO"
        }
        return $true
    }
    ...
    try {
        ...
        Update-FileMetadata -FilePath $DestinationPath -Metadata $Metadata
...
function New-OrganizedPath {
    ...
    # Create folder structure: Artist\Year - Album
    $folderPath = Join-Path $BaseOutputPath $artist
    if ($year -ne 'Unknown') {
